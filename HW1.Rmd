...
insur_fit <- glm(INS ~ DDA + DDABAL + DEP + CHECKS + TELLER + SAV + SAVBAL + ATMAMT + CD + CDBAL + MM,
...
exp(max(insur_fit$coefficients)) # MM has the largest estimate of 2.349813
exp(confint(insur_fit)) # Since we're exponentiating, these are the CIs 

keep_vars <- c("DDA", "DDABAL", "DEP", "CHECKS", "TELLER", "SAV", "SAVBAL", "ATMAMT", "CD", "CDBAL", "MM","INS")
...
# For the odds ratios, interepretations are 
   # MM Condifence Interval = (2.0229291, 2.7302445)
   # A person with a money market account is 2.35 as likely to buy the insurance product as someone who does not have a money market account. 
```
...
summary(compare_set)
new1 <- data.frame(DDA = c(1,1), DDABAL = c(563.42, 563.2), DEP = c(2.131,2.131), CHECKS = c(2,2), TELLER = c(0,0), SAV = c(0,0), SAVBAL = c(0,0),ATMAMT = c(119.9,119.9),CD=c(0,1),CDBAL=c(0,20000),MM=c(0,1))

exp(predict(insur_fit, newdata = new1, type = "link")) # Gets the prediction for each individual new test case.

exp(diff(predict(insur_fit, newdata = new1, type = "link"))) # Interpretation of test cases - An average person is 6.86 times less likely to have an annuity than someone who is average except for having a CD with balance $20,000 and an MM.
